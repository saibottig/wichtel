...
- Both `three.module.js` and `three.core.js` count. The first re-exports the second, and the second is the bigger half. Quoting only the first understates the cost by more than half: 421 kB gzip unminified for the pair, 204 kB for the minified pair plus the add-ons.
